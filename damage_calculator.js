...
function calculateDamage(config = null) {
    ...
    console.log('=== 技能信息 ===');
    const skillName = activeSkill === 1 ? '居合斩' : activeSkill === 2 ? '一闪' : '自定义';
    ...
    console.log('=== 伤害构成分析 ===');
    console.log(`物理攻击贡献: ${physContribution.toFixed(1)} (${((physContribution / (physContribution + elemContribution + refineContribution + addContribution)) * 100).toFixed(1)}%)`);
    console.log(`元素攻击贡献: ${elemContribution.toFixed(1)} (${((elemContribution / (physContribution + elemContribution + refineContribution + addContribution)) * 100).toFixed(1)}%)`);
    console.log(`精炼攻击贡献: ${refineContribution.toFixed(1)} (${((refineContribution / (physContribution + elemContribution + refineContribution + addContribution)) * 100).toFixed(1)}%)`);
    console.log(`附加伤害贡献: ${addContribution.toFixed(1)} (${((addContribution / (physContribution + elemContribution + refineContribution + addContribution)) * 100).toFixed(1)}%)`);
    console.log('');

    console.log('=== 乘区信息 ===');
    console.log(`公共乘区总加成: ${((normalPublicMultiplier - 1) * 100).toFixed(2)}%`);
